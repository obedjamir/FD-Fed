    ...
    def train(self):
        for i in range(self.global_rounds):
            ...
            if mean_test_acc >= self.best_mean_test_acc:
                ...
                for client in self.clients:
                    torch.save(client.model.state_dict(), f"Local/{self.args.dataset}-{self.args.model_name}-num_clients:{self.args.num_clients}-{client.id}-{self.args.goal}-{self.times}.pth")
            elif i >= 9:
        ...
        for client in self.clients:
            client.model.load_state_dict(torch.load(f"Local/{self.args.dataset}-{self.args.model_name}-num_clients:{self.args.num_clients}-{client.id}-{self.args.goal}-{self.times}.pth"))
